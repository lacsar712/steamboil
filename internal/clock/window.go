...
func NewPurgeWindow(clk ProcessClock) *PurgeWindow {
	return &PurgeWindow{clk: clk, window: NewWindow(model.PurgeWindow)}
}
...
type DrumSwellWindow struct {
	clk    ProcessClock
	window Window
}

func NewDrumSwellWindow(clk ProcessClock) *DrumSwellWindow {
	return &DrumSwellWindow{clk: clk, window: NewWindow(model.DrumSwellSettleWindow)}
}

func (d *DrumSwellWindow) Settled(swellAt time.Time) bool {
	return d.window.Satisfied(d.clk, swellAt)
}

func (d *DrumSwellWindow) RequireSettled(swellAt time.Time) error {
...
type CombustionWarmupWindow struct {
	clk    ProcessClock
	window Window
}

func NewCombustionWarmupWindow(clk ProcessClock) *CombustionWarmupWindow {
	return &CombustionWarmupWindow{clk: clk, window: NewWindow(model.CombustionWarmupWindow)}
}

func (c *CombustionWarmupWindow) Ready(ignitionAt time.Time) bool {
	return c.window.Satisfied(c.clk, ignitionAt)
}

func (c *CombustionWarmupWindow) Require(ignitionAt time.Time) error {

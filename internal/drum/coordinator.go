...
func NewCoordinator(clk clock.ProcessClock) *Coordinator {
	return &Coordinator{
		...
		settle:    clock.NewDrumSwellWindow(clk),
	}
}
...
func (c *Coordinator) Tick(ctx context.Context, snap model.PlantSnapshot, firing bool) (model.DrumReading, error) {
	select {
	case <-ctx.Done():
		return snap.Drum, fmt.Errorf("%w", model.ErrContextDone)
	default:
	}
	out := snap.Drum
	...
	if firing {
		out.SteamFlowTPH = snap.Boiler.MainSteamFlowTPH
	}
	out.CarryoverPPM = c.carryover.Estimate(out, snap.Boiler.SteamPressurePSI)
...
func (c *Coordinator) SettledAfterSwell(snap model.DrumReading) bool {
...
func (c *Coordinator) RequireSettled(snap model.DrumReading) error {
	if snap.LastSwellAt.IsZero() {
		return nil
	}
...
func (c *Coordinator) TripRequired(snap model.DrumReading) bool {
	return snap.LevelPercent < model.TripDrumLowPercent || snap.LevelPercent > model.TripDrumHighPercent
}

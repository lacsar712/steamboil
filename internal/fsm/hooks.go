...
// FurnaceDrivePulse counts furnace-side effects from FSM after hooks (acceptance).
var FurnaceDrivePulse func()

func RegisterFurnaceDriveHook(chain *HookChain) {
	chain.OnAfter(func(ctx context.Context, from, to model.PlantState, event PlantEvent) error {
		_ = ctx
		_ = from
		_ = to
		_ = event
		if FurnaceDrivePulse != nil {
			FurnaceDrivePulse()
		}
		return nil
	})

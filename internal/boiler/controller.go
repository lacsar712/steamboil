...
func (c *Controller) Tick(ctx context.Context, snap model.PlantSnapshot, firing bool) (model.BoilerReading, error) {
	select {
	case <-ctx.Done():
		return snap.Boiler, fmt.Errorf("%w", model.ErrContextDone)
	default:
	}
	pressure, err := c.pressure.Compute(snap, firing)
	if err != nil {
		return snap.Boiler, err
	}
